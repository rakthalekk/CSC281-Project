...
func _process(delta):
	if dialog_idx == 3 && parent.hudUnobtainiumCount >= 2 && $TransitionTimer.time_left == 0:
		...
		show_next_message()
	elif dialog_idx == 6 && parent.hudFairyDustCount == 3:
		show_next_message()
